...
class TestDecisionEngine(unittest.TestCase):
    def test_safe_commands(self):
        ...
        self.assertTrue(
            is_safe_command(
                "python response_management/reply_monitor.py --auto-approve"
            )
        )
        self.assertTrue(
            is_safe_command("python scale/campaign_manager.py --create new_campaign")
        )
        self.assertTrue(
            is_safe_command("python3 intelligence/lead_finder.py arg1 arg-2 arg_3")
        )
    ...
    def test_invalid_commands(self):
        ...
        self.assertFalse(
            is_safe_command("python3 ai_ceo/decision_engine.pyc")
        )  # slight variation

    def test_malicious_arguments(self):
        # Shell metacharacters
        self.assertFalse(
            is_safe_command("python intelligence/lead_finder.py && rm -rf /")
        )
        self.assertFalse(is_safe_command("python intelligence/lead_finder.py ; ls"))
        self.assertFalse(
            is_safe_command("python intelligence/lead_finder.py | grep test")
        )
        self.assertFalse(
            is_safe_command("python intelligence/lead_finder.py > output.txt")
        )
        self.assertFalse(
            is_safe_command("python intelligence/lead_finder.py < input.txt")
        )
        self.assertFalse(is_safe_command("python intelligence/lead_finder.py `whoami`"))
        self.assertFalse(
            is_safe_command("python intelligence/lead_finder.py $(whoami)")
        )
        ...
        self.assertFalse(
            is_safe_command("python intelligence/lead_finder.py --arg=value ; rm -rf /")
        )
