...
def deploy_site(business_slug: str, html_content: str) -> str:
    ...
    try:
        ...
        if res.returncode == 0:
            # Git add
            subprocess.run(
                ["git", "add", f"docs/sites/{file_name}"], capture_output=True
            )
            ...
            status = subprocess.run(
                ["git", "status", "--porcelain", f"docs/sites/{file_name}"],
                capture_output=True,
                text=True,
            )
            cname_status = subprocess.run(
                ["git", "status", "--porcelain", "docs/CNAME"],
                capture_output=True,
                text=True,
            )
            ...
            if status.stdout.strip() or cname_status.stdout.strip():
                ...
                print(
                    "   ℹ️ No changes to commit (site is identical to published version)"
                )
        else:
        ...
        print(
            "   ℹ️ Saved site locally. Make sure to commit and push the 'docs' folder to publish."
        )
