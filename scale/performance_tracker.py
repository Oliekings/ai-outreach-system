...
def calculate_lead_metrics(data: dict) -> dict:
    ...
    # Consolidate all metric calculations into a single O(N) iteration
    enriched = 0
    with_email = 0
    with_wa = 0
    with_ig = 0
    ...
    for lead in leads:
        ...
        if lead.get("contact_whatsapp"):
            with_wa += 1
        if lead.get("instagram", {}).get("found"):
            with_ig += 1
        if lead.get("facebook", {}).get("found"):
            with_fb += 1
        if lead.get("site_built"):
            site_built += 1
    ...
    avg_score = round(total_score / max(scores_count, 1), 1)
