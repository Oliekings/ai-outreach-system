...
def get_detailed_logs():
    # ⚡ Bolt Optimization: Use lexical string comparison to filter out logs older than the 12-day cutoff
    # without running full datetime parsing on every log entry. This significantly speeds up API response.
    from datetime import datetime, timedelta
    now = datetime.now()
    cutoff = now - timedelta(days=12)
    cutoff_iso = cutoff.isoformat()[:10]  # Just YYYY-MM-DD
    ...
    def parse_date(date_str):
        if not date_str:
            return None
        # Fast path for typical ISO formats generated by our system
        try:
    ...
    for entry in email_log.get("emails", []):
        ...
        # Fast lexical comparison to skip full parsing for definitely old logs
        if len(date_str) < 10 or date_str[:10] >= cutoff_iso:
