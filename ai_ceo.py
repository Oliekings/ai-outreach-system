...
# ─────────────────────────────────────────────────────────────────────────────
def run_ai_ceo(mode: str = "full", dry_run: bool = False, autonomous: bool = False):
    """
    Modes:
    ...
    if mode in ["full", "schedule"]:
        ...
        try:
            ...
            if generated > 0:
                print(
                    f"   {Symbol.CHECK} Generated {generated} new follow-up message(s)"
                )
            else:
    ...
    if mode in ["full", "decide"]:
        ...
        print(
            f"   🏥 Pipeline health: {decisions_result.get('pipeline_health', 'unknown')}"
        )
        ...
        if should_act_autonomous and not dry_run:
            ...
            for d in decisions_result.get("decisions", []):
                print(
                    f"      [{d.get('priority')}] {d.get('reason')} → {d.get('command', '')}"
                )
        else:
            print(
